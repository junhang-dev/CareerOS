...
- `career_profiles`, `career_experiences`, `career_projects`는 아직 조회 중심

## 다음 세션 권장 우선순위

1. `career profile / experience / project` writable CRUD
2. 첫 `job source adapter`와 `manual import` 진입점 추가
3. 문서 업로드 + 텍스트 추출 파이프라인 연결
4. stub 분석을 실제 워커/LLM 기반 분석으로 교체
