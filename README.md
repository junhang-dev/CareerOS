...
`career-assets` 쓰기 경로는 현재 `profile`, `experience`, `project`, `document` 입력을 모두 지원하며,
입력 검증과 응답 계약은 `memory`/`postgres`에서 같은 규칙을 따르도록 정리돼 있다.
...
제품 판단 기준은 `docs/product/product-principles.md`를 먼저 확인한다.
CareerOS의 현재 제품 방향은 외부 채용 플랫폼이 아니라, LinkedIn/리멤버/GitHub/포트폴리오/이력서에
보여줄 커리어 내러티브를 내부에서 정리하는 `docs/product/career-branding-studio.md` 기준으로 본다.
